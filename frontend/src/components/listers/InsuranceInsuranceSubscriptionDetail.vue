<template>
    <v-card outlined>
        ...
        <v-card-text>
            ...
            <div>
                <String label="TerminalProductId" v-model="item.terminalProductId" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/insuranceSubscriptions/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
